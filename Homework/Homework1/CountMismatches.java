...
public class CountMismatches {
    ...
    public static int count_mismatches(String seq1, String seq2) {
		//iterate through the strings. assume they have the same length to start. A:T, C:G
		int difference = 0;
		if (seq1.length() != seq2.length()){
			difference = Math.abs(seq1.length() - seq2.length()); //number of characters that are for sure mismatched - they have no counterpart
		}
		int longestString = Math.max(seq1.length(), seq2.length()); //make sure you iterate over the whole thing - difference. smaller length - difference is a problem.
		int errors = 0;
		for (int i = 0; i < longestString - difference; i++) {
			String c1 = seq1.substring(i, i + 1);
			String c2 = seq2.substring(i, i + 1);
			String together = c1 + c2;
			if (!together.equals("AT") && !together.equals("TA") && !together.equals("CG") && !together.equals("GC")){
				errors++;
			}
		}
