...
public class MaxOfBoth {
	...
	public static int max_of_both(int[] x, int[] y) {
		//go through both lists and make a list of number that appear in both. return the max of the new list.
		int numRepeats = 0;
		...
		int[] repeats = new int[numRepeats];
		int currentIndex = 0;
		for (int i = 0; i < x.length; i++) {
			for (int j = 0; j < y.length; j++) {
				if(x[i] == y[j]){
					repeats[currentIndex] = x[i];
					currentIndex++;
				}
			}
		}
